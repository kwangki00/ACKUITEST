...
/**
 * **폭은 부모가 정합니다** (`Input` 과 같은 규칙). 아래 예는 전부 `quickSelect` 입니다.
 *
 * ### 칩과 한 줄일 때만 입력창이 자기 폭을 갖습니다
 *
 * 그 줄은 입력창과 칩이 폭을 **나눠 갖는** 자리라, 입력창이 먼저 몫을 정하지 않으면
 * 칩이 밀려납니다 — `day` **248** · `month` **204** · `year` **156** (`PICKER_WIDTH`).
 * 자리가 모자라면 칩이 다음 줄로 내려가고 **각 줄이 꽉 찹니다**.
 *
 * `quickSelect` 를 끄면 이 폭은 쓰이지 않습니다. 그냥 필드일 때는 나눌 상대가 없어
 * `Input` · `Select` 처럼 부모를 따릅니다 — **`FormField` 안에서 날짜 칸만 혼자 짧으면
 * 줄이 어긋나 보입니다** (`Controls/DatePicker` 의 「폭」 스토리에 자세히 있습니다).
 */
