...
/**
 * **닫혀 있을 때도 방향키가 값을 바꿉니다** (2026-08-13) — 네이티브 `<select>` 와
 * 같습니다. 짝인 `NativeSelect` 가 그렇게 동작하는데, 같은 껍데기를 쓰고 바꿔 끼울 수
 * 있는 둘이 키보드에서 갈려 있었습니다.
 *
 * **끝에서 돌지 않습니다** — 마지막에서 아래를 눌러 첫 항목으로 튀면 값이 멀리 옮겨간
 * 것을 못 보고 지나칩니다. 열린 패널 안의 커서는 도는 것이 맞지만(짚기만 할 뿐 값이
 * 안 바뀝니다) 여기는 누를 때마다 **실제 값이 바뀝니다.**
 *
 * `Home`·`End` 는 처음·끝 · **`Alt+아래`는 패널을 엽니다**(표준) · 비활성은 건너뜁니다.
 * 눌러 보세요 — 탭으로 트리거에 온 뒤 위·아래를 누르면 값이 바뀝니다.
 */
