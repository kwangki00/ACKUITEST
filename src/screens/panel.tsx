 ...
 *
 * ### `screens/` 에 있고 `components/ui` 에는 없습니다
 *
 * 처음에는 `result-lookup/` 안에 두었습니다 — 그 화면에서만 썼기 때문입니다.
 * **`레이아웃1` 이 같은 판을 쓰게 되어 한 겹 위로 올렸습니다** (2026-08-13).
 *
 * 더 올리지는 않습니다. `components/ui` 는 Figma 와 1:1 로 대응하는 자리인데
 * 이건 대응물이 없고, 올리면 스토리·변형 축을 갖춘 부품으로 다듬어야 합니다.
 * **화면 둘이 쓰는 화면 조립 부품**이라 이 자리가 맞습니다.
 *
 * ### `Card` 가 아닙니다
 *
