...
- **`paginated` 를 꺼도 `table.getRowModel()` 은 페이지네이션을 거칩니다** (2026-08-13 수정). `rowPaginationFeature` 가 features 에 늘 들어 있어서 마지막 행 모델이 항상 잘렸고, TanStack 기본 `pageSize` 가 10 이라 **200건을 넘겨도 10행만 나오고 아래는 조용히 사라졌습니다.** 쪽 버튼은 `paginated` 일 때만 그리므로 화면에는 「10행짜리 표」로 보일 뿐 잘렸다는 신호가 없습니다
  - 기능을 빼서 고칠 수 없습니다 — `paginated` 를 켜는 곳이 **같은 features** 를 씁니다. 그래서 자를지 말지를 렌더에서 고릅니다: `paginated ? getRowModel() : getSortedRowModel()`
...
- **닫혀 있을 때 방향키가 값을 바꿉니다** (2026-08-13). 네이티브 `<select>` 와 같습니다 — 짝인 `NativeSelect` 가 Windows 에서 그렇게 동작하는데, **같은 껍데기를 쓰고 바꿔 끼울 수 있는 둘이 키보드에서 갈려** 있었습니다. 그전에는 방향키 처리가 패널에만 달려 있어 닫힌 트리거는 방향키를 아예 안 받았습니다
  - **끝에서 돌지 않습니다.** 마지막에서 아래를 눌러 첫 항목으로 튀면 값이 멀리 옮겨간 것을 못 보고 지나칩니다 — 열린 패널 안의 커서는 도는 것이 맞지만(짚기만 할 뿐 값이 안 바뀝니다) 여기는 누를 때마다 **실제 값이 바뀝니다**
  - `Home`·`End` 는 처음·끝 · **`Alt+아래`는 패널을 엽니다**(표준) · 비활성 항목은 건너뜁니다
  - **`onValueChange` 가 한 칸마다 불립니다.** 지금은 조회 버튼이 따로 있어 안전하지만, 고르는 즉시 서버를 치는 자리가 생기면 그때 다시 봐야 합니다
  - **`Combobox` · `Lookup` 에는 넣지 않았습니다** — 검색이 붙는 컨트롤이라 타이핑이 먼저고, 다중은 「다음 값」이라는 개념이 없습니다. `editable` 은 트리거가 입력창이라 방향키가 값을 갈아끼우면 **치던 글자를 덮어씁니다** — 목록을 열어 짚고 Enter 로 고릅니다
    - 다만 **닫혀 있으면 방향키가 패널을 엽니다** (2026-08-13). 고르면 패널이 닫히는데 커서는 입력창에 남아서, 그 상태의 방향키가 **안 보이는 커서만 움직이고** 화면은 아무 반응이 없었습니다 — 누른 사람은 고장으로 읽습니다. 열면 커서는 고른 값에 갑니다
...
|---|---|---|
| 조건 배치 | 세로로 쌓음 | **가로 한 줄** |
| 접힌 줄 | 요약 (칩이 **줄바꿈**) | 요약 + **“조건 변경” 버튼** |
| 캡션 | 펼치면 커짐(`base/SemiBold`) | 늘 `sm` |
| 버튼 | 화면을 반씩 (`flex-1`) | 우측에 내용 폭만큼 |
| 누르는 곳 | **줄 전체** | 버튼만 |

- **조회하면 자동으로 접힙니다.** 조건은 한 번 정하고 결과를 계속 봅니다. 접으면 **표가 약 150px 넓어집니다** (펼침 200 · 접힘 56)
  - **처음에는 펼쳐 둡니다** (2026-08-13). 화면에 들어와서 가장 먼저 하는 일이 조건을 거는 것인데, 접혀 있으면 그 전에 「조건 변경」을 한 번 눌러야 합니다 — 아직 아무것도 안 한 사람에게 접힌 요약을 보여줄 이유가 없습니다. 두 화면(`결과조회` · `레이아웃1`) 모두 그렇습니다
...
- **펼치면 캡션이 주인공, 접히면 요약이 주인공입니다** (2026-08-13 정리). 펼쳤을 때 그 줄은 영역의 머리라 캡션이 제목처럼 서지만, 접히면 캡션은 **안 변하는 라벨**이라 알릴 것이 없고 걸린 조건이 그 줄의 내용입니다

  | | 캡션 「조회 조건」 | 요약 |
  |---|---|---|
  | 펼침 | `SemiBold` · `Text/Basic` | (없음) |
  | 접힘 | Regular · `Text/Subtle` | **`Medium` · `Text/Basic`** |

  **넓을 때가 반대로 되어 있었습니다** — 캡션이 SemiBold·`Text/Basic` 이고 요약이 Regular·`Text/Subtle` 이라, 매번 바뀌는 값이 안 바뀌는 라벨보다 흐렸습니다. 좁을 때는 처음부터 지금 모양이었습니다
- **요약은 접혔을 때만.** 펼치면 아래 필드에 같은 정보가 있어 중복입니다
- **건수 배지는 없앴습니다** (2026-08-13). 좁을 때만 나오던 것인데, **바로 아래 `MobileListHeader` 가 같은 값을 이미 말합니다** — 한 화면에 두 곳에 나오면 다를 때 어느 쪽이 맞는지 판단할 수 없습니다. `count` prop 도 함께 지웠습니다
- **조건 하나가 칩 하나입니다** (2026-08-13). `summary` 에 `{ icon, label, value }[]` 를 넘깁니다. `·` 로 이으면 **어디까지가 한 조건인지 눈이 매번 재야 합니다** — 값이 두 낱말 이상이면 더 그렇습니다

  | 값이 | 어떻게 | 예 |
  |---|---|---|
  | 스스로 말함 | `{ value }` | `2026-07-14 ~ 2026-08-13` · `일반혈액검사` · `완료` |
  | **스스로 못 말함** | `{ icon, label, value }` | 👤 `이검사` · 🔍 `김지훈` |
  | 아이콘이 마땅찮음 | `{ label, value }` | `상태 완료` |

  - **`이검사`(담당자)와 `김지훈`(검색어)이 문제였습니다** — 사람 이름 둘이 나란히 서면 어느 쪽이 무엇인지 알 수 없어, 접힌 줄이 「지금 뭐가 걸려 있나」를 알려주는 일을 못 합니다
  - **아이콘을 남발하지 마세요.** 검사항목·상태처럼 쓸 아이콘이 마땅찮은 자리에 억지로 붙이면 **결국 눌러 봐야 알게 되어** 아무것도 없는 것보다 나쁩니다 (`MBottomTabBar` 의 「라벨을 빼지 마세요 — 아이콘만으로는 통계조회와 검사이력이 구분되지 않습니다」와 같은 이유)
  - **아이콘을 줄 때 `label` 도 함께 주세요** — 화면에서는 숨고(`sr-only`) **보조기술이 읽습니다.** 안 주면 스크린리더로는 이름 두 개가 그대로 나열되어, 고친 문제가 그대로 남습니다
  - **`Chip` 이 아니라 `Badge`** 입니다 — 이 시스템에서 `Chip` 은 **지울 수 있는 태그**라 없는 X 를 찾게 됩니다. 크기는 `lg`(28 · 14px). `default`(24 · 12px)면 한글이 뭉개집니다(`FormField` 라벨을 12→14 로 올린 것과 같은 이유)
  - **넓으면 잘리고 좁으면 줄바꿈합니다** (2026-08-13). 넓은 머리줄은 높이 32 **고정**이라 줄바꿈하면 줄이 통째로 커지지만, 좁을 때는 높이를 안 잡고 여백(`py-3`)으로만 잡아 늘어나도 됩니다 — 폭이 좁아 칩 서넛이면 이미 넘치는데 거기서 뒤를 잘라내면 **걸린 조건을 아예 볼 수 없습니다.** 칩은 `shrink-0` 이라 찌그러지지 않습니다
  - 문자열도 그대로 받습니다 — 칩이 필요 없는 곳과 문서용입니다
  - **「전체」는 담지 마세요** — 조건을 건 게 아니라 안 건 것이라, 넣으면 `전체 · 전체 · …` 가 되어 무엇을 걸었는지가 오히려 안 보입니다
...
src/screens/panel.tsx              Panel · SectionTitle · TableFrame (화면 둘이 씁니다)
src/screens/layout-1/              **레이아웃1** — 위 조회 조건 · 아래 목록 표
...
- **`panel.tsx` 는 `screens/` 에 있습니다** — 처음에는 `result-lookup/` 안이었고 "다른 화면에서 같은 모양이 필요해질 때 올린다" 고 적어뒀는데, **레이아웃1 이 같은 판을 쓰면서 그날이 왔습니다** (2026-08-13). 더 올리지는 않았습니다 — `components/ui` 는 Figma 와 1:1 인 자리인데 이건 대응물이 없고, 올리면 스토리·변형 축을 갖춘 부품으로 다듬어야 합니다
- **`SCREENS` 는 그리는 것과 틀(`className`)을 함께 갖습니다** — 스크롤을 누가 갖느냐가 화면마다 다릅니다. 갤러리는 긴 문서라 탭 패널이 통째로 스크롤되고, 레이아웃1 은 안의 표가 스스로 스크롤합니다. 여기 함께 적지 않으면 화면을 더할 때마다 `TabPanel` 을 고치게 됩니다
...
- **갤러리는 메뉴에 넣었습니다** — 사이드바 맨 아래 `컴포넌트`. 빌드된 앱에서 토큰과 컴포넌트가 실제로 나오는지 눈으로 보는 자리입니다. Storybook 은 별도 빌드라 배포본에 없습니다. **테스트 빌드 전용이니 실제 제품에서는 `MENU` 의 그 줄을 지우세요** — `레이아웃` 도 같습니다

### 레이아웃1 — 위 조회 조건 · 아래 목록 표 (2026-08-13)

**이 제품에서 가장 흔한 화면 모양**입니다. `결과조회` 가 좌우로 갈린 것은 **환자를 옮겨 다녀야 해서**고, 그 흐름이 없는 화면은 전부 이 모양입니다 — 조건을 정하고, 훑고, 한 줄을 골라 무언가를 합니다. 새 화면은 여기를 보고 베끼세요 (`사이드바 → 레이아웃 → 레이아웃1`).

```
Content
├ FilterBar        조회하면 접힙니다 (200 → 56)
└ 본문 p-5
   └ Panel gap-0   제목줄 · 표 · 페이지네이션 한 덩어리
```

`결과조회` 와 **갈리는 것은 둘뿐**입니다.

| | `결과조회` | **레이아웃1** |
|---|---|---|
| 판 | 좌우 둘 (540 + 나머지) | **하나** — 폭을 다 씁니다 |
| 쪽 나눔 | `Pagination` 을 스크롤 밖에 | **없음** — 200건이 아래로 이어집니다 |

- **쪽을 나누지 않습니다** — 「한 화면에서 훑는 목록은 스크롤, 몇 번째 쪽을 보고 있었는지를 기억해야 하면 페이지네이션」 규칙의 **앞쪽**입니다. 조건을 좁혀 가며 훑는 화면이라 쪽을 오갈 일이 없고, 쪽이 있으면 「조건을 더 걸까 다음 쪽으로 갈까」가 매번 판단거리가 됩니다
  - 대신 **`stickyHeader` 가 필수**입니다. 스크롤은 **`framed` 상자가 갖습니다** — 테두리를 만드는 쪽이 스크롤을 가져야 표가 길어져도 테두리·모서리가 제자리에 남습니다 (2026-08-13. 그전에는 `overflow-hidden` 이라 아예 잘렸습니다)
  - **서버 페이지네이션이 붙는 날**은 바깥에 `Pagination` 을 두세요. `결과조회` 가 그 모양인 이유는 서버가 한 쪽씩 주기 때문입니다(전체 979 · 받은 것 14) — 받은 배열을 자르면 안 됩니다
- **목록이 긴 조건은 `editable` 입니다** — 검사항목 25종 · 담당자 12명은 `Select` 로 못 고릅니다. 목록을 끝까지 내려야 하고, 이름을 알아도 눈으로 찾아야 합니다. `Combobox render="editable"` 은 아는 이름을 쳐서 거르고 모르면 열어서 훑습니다. 상태(3종)는 그대로 `Select` — 항목보다 검색창이 더 큽니다
  - **목록에 「전체」를 두지 않습니다.** `editable` 은 **비어 있는 것이 곧 「전체」** 라 자리표시가 그렇게 적혀 있고 지우기(X)로 언제든 돌아갑니다. 항목으로 두면 쳐서 거를 때 검사항목처럼 걸려 나옵니다
- **조건은 조회 버튼을 눌러야 반영됩니다** — `query`(치는 중) 와 `applied`(적용된 것)를 따로 둡니다. 치는 동안 표가 계속 바뀌면 무엇을 보고 있었는지 놓칩니다
- 요약에는 **걸린 조건만** 씁니다 — 「전체」는 조건이 아니라 안 건 것이라 빼야 요약이 짧아집니다
- 완료일이 빈 행은 **빈칸이 아니라 `-`** 입니다. 빈칸은 값이 빠진 것인지 없는 것인지 구분되지 않습니다
- 편집 버튼은 **한 줄만 골랐을 때**입니다 — 여럿을 한 번에 고치는 것은 별도 화면이 있어야 하는 동작이라, 여기서 열면 무엇이 바뀌는지 알 수 없습니다
