...
**굵기만으로는 안 됩니다** — 10px 을 올리면 굵기로 상태를 알리던 곳이 딸려 무너집니다. 원인(글꼴)을 먼저 바꾸고, 굵기는 **작은 두 크기**(10 · 12)만 손봤습니다.

### 10px · 12px 은 기본 굵기가 SemiBold 입니다

한글은 획이 많아 이 크기에서 먼저 뭉갭니다. Medium 으로 먼저 올렸다가 **여전히 흐려서 한 단계 더** 갔습니다. **크기 토큰이 굵기를 함께 갖습니다** — `--text-2xs--font-weight: 600` · `--text-xs--font-weight: 600` (Tailwind 4 기능).

**14px 부터는 Regular 입니다** — 작은 두 크기만 굵기로 버팁니다.
...
- **12px 도 SemiBold 입니다** (2026-08-12). 처음에는 「12px 은 Regular 로 둔다」고 적었습니다 — `FormField` 의 설명·에러(12)가 굵어져 14 라벨과의 위계가 흐려진다는 이유였습니다. **읽히는 것이 먼저라 뒤집었습니다**: 12px 한글도 Windows 에서 뭉개집니다. 위계는 굵기 말고 **위치와 색**이 이미 나누고 있습니다 (설명·에러는 컨트롤 아래, 색도 다릅니다)
...
- **머리줄 아래 선은 `border` 가 아니라 `inset` 그림자입니다** (2026-08-13). 표가 `border-collapse: collapse` 라 테두리가 **칸이 아니라 표의 격자**에 그려지는데, `stickyHeader` 로 머리줄이 떠도 **선은 따라오지 않고 제자리에 남습니다** — Chrome 에서는 아예 안 그려집니다. 머리줄에 아래 선이 없으면 첫 행이 붙어 어디까지가 열 이름인지 흐려집니다
  - `inset` 그림자는 **칸 안쪽**에 그려서 칸을 따라 움직입니다. 굵기·색은 같고 `border-box` 라 높이도 그대로입니다
  - **몸통 행은 `border-b` 그대로**입니다 — 뜨지 않으니 격자에 그려도 됩니다
...
- **아이콘 크기를 배지가 정합니다** (2026-08-13) — sm 12 · default 14 · lg 16. 안 정하면 lucide 기본값 24 로 나와 배지보다 커집니다. 글자 옆 아이콘이라 `Button` 과 같은 비율입니다
  - **호출부에서 `size-*` 를 주지 마세요.** 실제로 `FilterBar` 가 조건 칩에 `[&>svg]:size-3.5` 를 손으로 붙이고 있었습니다 — 배지를 쓰는 다른 자리에 아이콘을 넣으면 거기서 또 빠집니다
  - Figma 는 `Icon`(보임) + `IconType`(무엇) 두 프로퍼티입니다. 코드는 children 이라 슬롯이 필요 없지만 **그림은 노드가 있어야** 그립니다. 기본은 꺼짐이라 기존 배지는 그대로입니다
  - 색은 라벨과 **같은 변수**에 묶여 톤을 바꾸면 함께 따라옵니다. **`Dot` 과 함께 켜지 마세요** — 표식이 둘이 됩니다
...
- **handlebar 는 끌어내려 닫을 수 있다는 신호**라 실제로 드래그가 동작합니다. 신호만 주고 안 되면 한 번 해보고 다시는 시도하지 않습니다. 높이의 1/4(최소 80px) 이상 내리면 닫힙니다 — 절반으로 잡으면 큰 시트를 닫기 힘듭니다
...
- handlebar 색은 **`Sheet/Handle`** 입니다 (코드 `--color-sheet-handle`). Figma 가 `Color/gray/300` 을 직접 참조하고 있어 Semantic 변수를 새로 만들어 바인딩했습니다 (2026-08-07). 회색 팔레트를 손대면 handlebar 까지 함께 바뀌는 걸 막습니다
...
- **키보드가 올라오면 시트가 그만큼 올라갑니다** (2026-08-13). 시트는 `fixed … bottom-0` 이라 **레이아웃 뷰포트**의 바닥에 붙는데, 키보드가 떠도 그 뷰포트는 그대로인 브라우저가 있습니다 — 그러면 시트는 제자리에 남고 **목록과 확인 버튼이 키보드 밑에 깔립니다.** 시트 안에 검색창이 있는 `MobileSelect` 에서 바로 드러납니다
  - `index.html` 의 **`interactive-widget=resizes-content`** 가 Chromium 을 해결합니다 — 레이아웃 뷰포트가 키보드만큼 줄어듭니다. **iOS Safari 는 지원하지 않습니다**
  - 그래서 `MobileSheet` 가 **`visualViewport`** 로 따로 잽니다 — 레이아웃 뷰포트와의 차이가 곧 키보드 높이입니다. 그만큼 `bottom` 을 띄우고 최대 높이도 줄입니다. 위 meta 가 듣는 브라우저에서는 `innerHeight` 도 함께 줄어 차이가 0 이라 **겹치지 않습니다**
  - **`min-h-60`(240)을 함께 풀어야 합니다** — 남은 자리보다 크면 최소 높이가 이겨서 아래가 도로 잘립니다
  - **80px 아래는 무시합니다** — 주소줄이 접혔다 펴지는 것까지 키보드로 읽으면 스크롤할 때마다 시트가 들썩입니다
...
- **검색으로 결과가 줄어도 목록 칸은 240 을 지킵니다** (2026-09-14). 시트는 `bottom-0` 에 붙어 있어 내용이 줄면 **위쪽 가장자리가 내려옵니다** — 바로 그 위에 검색창이 있어서, 「혈액」 을 치는 동안 입력창이 손가락 밑에서 움직입니다. `ComboboxPanel` 의 `stableHeight` 가 목록에 `min-h-60` 을 세워 기준점을 고정합니다
  - 값이 `max-h-60` 과 **같은 240** 이라 「검색 있는 시트는 목록칸 240 고정」 한 줄로 설명됩니다
  - **「결과 없음」 에도 함께 세웁니다** — 0건이 가장 크게 줄어드는 자리라, 목록만 잡으면 거기서 도로 흔들립니다
  - **검색이 있을 때만 켭니다** (`stableHeight={searchable}`) — 검색이 없으면 목록이 줄어들 일이 없고, 항목 셋짜리 시트에 240 을 세우면 빈칸만 남습니다
  - **PC 팝오버는 켜지 마세요** — 팝오버는 트리거에 붙어 **위에서 아래로** 자라서 줄어들어도 기준점이 제자리입니다. 거기에 최소 높이를 세우면 항목 둘짜리 목록에 빈 240 이 생깁니다
...
- **그 버튼은 정렬 시트를 엽니다** (2026-08-27). 전에는 바로 위 `FilterBar` 를 여닫았는데, 좁을 때는 **그 줄 전체가 이미 누름 대상**이라 같은 일을 하는 장치가 둘이었습니다. 아이콘(`SlidersHorizontal`)도 다른 것을 기대하게 만듭니다
  - 이 줄이 있는 이유가 「표 헤더가 없어서」인데, PC 표가 헤더에서 하던 일 중 카드 목록에 자리가 없는 것이 **정렬**입니다
  - 정렬은 `FilterBar` 에도 있습니다(조건 다섯 중 하나). 값은 `query.sort` **하나**라 어긋나지 않고, 시트에서는 **고르는 즉시 반영**합니다 — 접힌 조건을 펴서 고르고 조회를 누르는 왕복을 줄이는 것이 이 버튼의 존재 이유입니다
...
- **좁을 때만 자동으로 접힙니다** (2026-08-13. 그전에는 폭과 무관하게 접었습니다). 접으면 **표가 약 150px 넓어집니다** (펼침 200 · 접힘 56)
  - **넓으면 접지 않습니다** — 세로가 넉넉한데 조회를 누를 때마다 접히면 다음 조건을 걸려고 매번 다시 펴야 합니다. 조회를 자주 누르는 화면에서 그 왕복이 제일 거슬립니다
  - **좁으면 접습니다** — 펼친 조건이 390×844 중 260 을 먹어서, 안 접으면 조회하고도 결과가 화면에 안 들어옵니다
  - **`FilterBar` 가 `ResizeObserver` 로 자기 폭을 잽니다.** 배치는 컨테이너 쿼리(CSS)가 정하지만 **접을지 말지는 동작**이라 JS 가 알아야 합니다. 경계는 배치와 **같은 `--container-pc`(880)** 이고 값을 코드에 적지 않고 토큰에서 읽습니다 — 두 벌이 되면 한쪽만 바뀝니다
  - **호출부에서 또 접지 마세요.** 화면 둘이 `onSearch` 에서 `onOpenChange(false)` 를 한 번 더 부르고 있었는데, 그러면 넓을 때 안 접는다는 판단을 덮습니다
...
시트 높이는 **최소 240 · 최대 717(화면의 85%)**. 최대를 넘으면 뒤 화면이 안 보여 임시 레이어라는 느낌이 사라집니다.
...
- **좁아지면 표만 카드로 갈립니다** (2026-08-13). **조회 조건은 그대로**입니다 — `FilterBar` 가 자기 폭을 재서 세로로 쌓습니다. 9열 200행을 390 폭에 넣으면 가로로 밀려 무엇을 보는지 알 수 없어서, 「아직 갈리는 것」의 `Table` ↔ `MobileListCard` 그대로입니다
  - 기준은 **창 폭 1024**(`useIsMobileLayout`) — 시트냐 팝오버냐를 정하는 `PointerModeProvider`(포인터)와는 **다른 축**이니 섞지 마세요
  - **모바일 껍데기가 고른 화면을 안 그리고 있었습니다.** `mobile.tsx` 가 `SCREENS` 를 읽지 않아, 좁은 화면에서 `레이아웃1` 을 골라도 **제목만 바뀐 환자 목록**이 나왔습니다 — 화면은 멀쩡해 보이는데 다른 것을 보고 있는 종류입니다. `screen` prop 으로 PC 와 같은 목록을 읽습니다
...
### `sticky` 는 `border-collapse` 의 테두리를 데려가지 않습니다

표의 머리줄을 `sticky` 로 띄웠는데 **아래 선만 안 따라왔습니다** (2026-08-13).

`border-collapse: collapse` 에서 테두리는 **칸이 아니라 표의 격자**에 그려집니다. 칸을 띄워도 선은 표에 남아 있고, Chrome 은 아예 그리지 않습니다. 배경(`bg-*`)은 칸에 그려지므로 **머리줄 색은 멀쩡한데 선만 사라져** 알아채기 어렵습니다.

- 답은 **`inset` 그림자**입니다 — 칸 안쪽에 그려서 칸을 따라 움직입니다. `shadow-[inset_0_-1px_0_var(--color-…)]`
- **뜨지 않는 줄은 `border` 그대로** 두세요. 그림자는 굵기·색을 토큰으로 못 받고 임의값이라, 필요한 자리에만 씁니다
...
- [ ] **Publish 가 다시 필요합니다** (2026-08-13) — `Badge` 에 `Icon` · `IconType` 프로퍼티가 생겼고 `PCFilterBar` · `MobileFilterBar` 의 접힘이 칩으로 바뀌었습니다. **새 프로퍼티는 발행 전까지 소비 파일에서 아예 안 보입니다**
