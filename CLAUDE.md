...
- **`quickSelect` 는 자리가 모자라면 칩이 다음 줄로 내려갑니다** (`flex-wrap`). 입력창 248(day) + 칩 넷은 한 줄에 **460** 쯤 필요합니다. **컨테이너 쿼리를 쓰지 않습니다** — 그건 폭을 부모에서 받아야만 성립해서, 폭 없는 자리(내용만큼 넓어지는 flex 항목)에 놓으면 **조용히 무너집니다.** `flex-wrap` 은 그런 조건이 없어 어디에 놓아도 동작합니다 (2026-08-07)
  ...
  - **폭은 컴포넌트가 갖습니다 — `Input` 과 다른 점입니다** (2026-08-11). 날짜는 자릿수가 정해져 있어 부모가 정해줄 이유가 없습니다. 단위마다 다릅니다 (`PICKER_WIDTH`)
    ...
    필요 폭 = 여백 24 + 글자 + 간격 8 + 우측 아이콘. 우측은 값이 있으면 지우기 16 + 4 + 달력 16 = **36**, 비어 있으면 달력 **16** 뿐이라 **`day` 는 자리표시가, 나머지는 값이** 폭을 정합니다 (Pretendard 14 실측). 여기에 4px 만 얹었습니다
    - **Figma 값을 따르지 않습니다** — 그림이 지우기 버튼을 그리지 않아 실제보다 좁게 잡히고(`month` 3px 모자람), 반대로 넉넉한 자리는 그대로였습니다(`year` 29px 남음)
    - 호출부에서 폭을 감싸지 마세요. 직접 정하려면 `className` 으로 덮으면 됩니다 — `w-full` 도 됩니다 (tailwind-merge 가 기본 폭을 걷어냅니다)
