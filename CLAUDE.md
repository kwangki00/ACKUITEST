...
- **요약에는 라벨을 함께 씁니다** (2026-08-13). `summary` 에 `{ label, value }[]` 를 넘기면 **라벨은 `Text/Subtle`, 값은 `Text/Basic Medium`** 으로 나옵니다 — 눈은 진한 값만 훑고 라벨은 필요할 때만 읽힙니다
  - **값만 나열하면 무엇을 걸었는지 알 수 없습니다.** `… · 이검사 · 김지훈` 에서 어느 쪽이 담당자고 어느 쪽이 검색어인지 구분되지 않았습니다 — 접힌 줄의 일이 「지금 뭐가 걸려 있나」인데 그걸 못 합니다
  - **헷갈리는 것만 붙이지 않습니다.** 무엇이 헷갈리는지는 **자료가 정하는 것**이라(검색어에 차트번호를 치면 안 헷갈리고 이름을 치면 헷갈립니다) 조건마다 사람이 판단하게 두면 새 조건이 붙을 때 빠집니다
  - **라벨은 위 필드의 것과 같은 말**입니다. 펼쳤을 때와 이어져야 어디를 고쳐야 하는지 바로 찾습니다
  - 문자열도 그대로 받습니다 — 라벨이 필요 없는 곳과 문서용입니다
